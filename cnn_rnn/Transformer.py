...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.embedding_pretrained is not None:
            ...
            self.embedding = nn.Embedding(config.vocab_size, config.embedding_size, padding_idx=config.vocab_size - 1)
        self.postion_embedding = Positional_Encoding(config.embedding_size, config.max_seq_length, config.dropout, config.device)
        ...
        self.fc1 = nn.Linear(config.max_seq_length * config.dim_model, config.num_classes)
